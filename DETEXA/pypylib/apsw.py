...
def complete(*args):
    return sqlite.sqlite3_complete(args[0].encode())

def apswversion():
    return "MSPW ver 0.01"

def sqlitelibversion():
    return ffi.string(sqlite.sqlite3_libversion()).decode()
...
class Connection(object):
    def __init__(self, database, timeout=None, detect_types=0, isolation_level="",
        ...
        if sqlite.sqlite3_open(str(database).encode(), db_p) != SQLITE_OK:
    ...
    def _get_exception(self, error_code=None):
        ...
        error_message = ffi.string(sqlite.sqlite3_errmsg(self._db)).decode()
        ...
        if "Error:" not in memoryview(error_message.encode()):
            error_message = exc.__name__ + ": " + str(error_message)
    ...
    def create_collation(self, name, callback):
        ...
        if callback is None:
            ...
            def collation_callback(context, len1, str1, len2, str2):
                text1 = ffi.string(str1).decode()
                text2 = ffi.string(str2).decode()
        ...
        if callback != None:
            ...
            try:
                ...
                def authorizer(userdata, action, arg1, arg2, dbname, source):
                    try:
                        arg1 = ffi.string(arg1).decode()
                    except:
                        arg1 = None
                    try:
                        arg2 = ffi.string(arg2).decode()
                    except:
                        arg2 = None
                    try:
                        dbname = ffi.string(dbname).decode()
                    except:
                        dbname = None
                    try:
                        source = ffi.string(source).decode()
                    except:
                        source = None
    ...
    def createscalarfunction(self, name, callback, num_args=-1):
            ...
            def closure(context, nargs, params):
                ...
                if nargs == 0:
                    ...
                    if t == 1:
                        ...
                        inparams = ffi.string(sqlite.sqlite3_value_text(params[0])).decode()
                    elif t == 4:
                        inparams = ffi.buffer(sqlite.sqlite3_value_blob(params[0]), sqlite.sqlite3_value_bytes(params[0]))[:].decode()
                    else:
                        inparams = None
                ...
                while ci < nargs:
                    ...
                    if t == 1:
                        ...
                        inparams.append(ffi.string(sqlite.sqlite3_value_text(params[ci])).decode())
                    elif t == 4:
                        inparams.append(ffi.buffer(sqlite.sqlite3_value_blob(params[ci]), sqlite.sqlite3_value_bytes(params[ci]))[:].decode())
                    else:
        ...
        try:
            ret = sqlite.sqlite3_create_function(self._db, name.encode(), num_args,
    ...
    def createmodule(self, name, datasource):
        ...
        try:
            ...
            def xCreate(db, paux, argc, argv, ppvtab, pzErr): #int (*xCreate)(sqlite3*, void *pAux, int argc, const char *const*argv, sqlite3_vtab **ppVTab, char**);
                try:
                    schema, table = datasource.Create(self, ffi.string(argv[0]).decode(), ffi.string(argv[1]).decode(), ffi.string(argv[2]).decode(), *tuple([ffi.string(argv[i]).decode() for i in range(3,argc)]))
                except Exception as e:
                    error_msg = str(e)
                    if "Error:" not in error_msg:
                        error_msg = type(e).__name__ +": " + error_msg
                    pzErr[0] = sqlite.sqlite3_mprintf(error_msg.encode())
                    return SQLITE_ERROR

                newvtab = ffi.new("sqlite3_vtab *")
                ppvtab[0] = newvtab
                self._vttables[newvtab] = table
                vret = sqlite.sqlite3_declare_vtab(db, schema.encode())
                if ret != SQLITE_OK:
            ...
            def xBestIndex(pvtab, pInfo): #int (*xBestIndex)(sqlite3_vtab *pVTab, sqlite3_index_info*);
                ...
                for i in range(pInfo.nOrderBy):
                    orderby.append((pInfo.aOrderBy[i].iColumn, pInfo.aOrderBy[i].desc))

                for ci in range(pInfo.nConstraint):
                ...
                if idxStr:
                    pInfo.idxStr = sqlite.sqlite3_mprintf(idxStr)
            ...
            def xFilter(vtabcursor, idxnum, cidxstr, argc, argv) : # int (*xFilter)(sqlite3_vta788b_cursor*, int idxNum, const char *idxStr, int argc, sqlite3_value **argv);
                try:
                    if cidxstr == ffi.NULL:
                        ...
                        idxstr = ffi.string(cidxstr).decode()
                    ...
                    while ci < argc:
                        ...
                        if t == 1:
                            ...
                            constraints.append(ffi.string(sqlite.sqlite3_value_text(argv[ci])).decode())
                        elif t == 4:
                            constraints.append(ffi.buffer(sqlite.sqlite3_value_blob(argv[ci]), sqlite.sqlite3_value_bytes(argv[ci]))[:].decode())
                        else:
                    ...
                    vtabcursor.pVtab.zErrMsg = sqlite.sqlite3_mprintf(str(e).encode('utf-8'))
                    return SQLITE_ERROR
            ...
            def xColumnFast(vtabcursor, context, col):
                #_python_to_sqlitedict[type(str(self._vtcursorinstances[vtabcursor.n].row[col]))](context, self._vtcursorinstances[vtabcursor.n].row[col])
                #return 0
                 _val = self._vtcursorinstances[vtabcursor.n].row[col]
                 cl = type(_val)
                 if cl is bytes:
                     sqlite.sqlite3_result_text(context, _val, -1, SQLITE_TRANSIENT)
                 else:
...
#  );
        ...
        ret = sqlite.sqlite3_create_module(self._db, name.encode() , vtmodule ,ffi.NULL)
        if ret != SQLITE_OK:
            raise self._get_exception(ret)
    ...
    def __iter__(self):
        while not self.exhausted:
            ...
            l_sqlite_to_python_statement = tuple([
                ...
                lambda st, c: ffi.string(sqlite.sqlite3_column_text(st, c)).decode(),
                lambda st, c: ffi.buffer(sqlite.sqlite3_column_blob(st, c), sqlite.sqlite3_column_bytes(st, c))[:].decode(),
                lambda st, c: None
            ])
...
class Statement(object):
    def __init__(self, connection, sql):
        ...
        self.next_char = ffi.string(mynext_char[0]).decode()
    ...
    def _get_description(self):
        if self.kind == DML:
            return ()
        desc = []
        for ci in range(sqlite.sqlite3_column_count(self.statement)):
            name = ffi.string(sqlite.sqlite3_column_name(self.statement, ci)).decode().split("[")[0].strip()
...
def _sqlite_to_python(val):
    ...
    if t < 2:
        ...
        if t == 3:
            return ffi.string(sqlite.sqlite3_value_text(val)).decode()
        else:
            if t == 4:
                return ffi.buffer(sqlite.sqlite3_value_blob(val), sqlite.sqlite3_value_bytes(val))[:].decode()
            else:
                return None
...
def _python_to_sqlite(context, val):
    cl = type(val)
    if cl is str:
        sqlite.sqlite3_result_text(context, val.encode(), -1, SQLITE_TRANSIENT)
    elif cl is int:
        sqlite.sqlite3_result_int64(context, val)
    elif cl is float:
        sqlite.sqlite3_result_double(context, val)
        ...
        sqlite.sqlite3_result_blob(context, str(val).encode(), len(val), SQLITE_TRANSIENT)

_sqlite_to_python_value = tuple([
    None,
    sqlite.sqlite3_value_int64,
    sqlite.sqlite3_value_double,
    lambda x: ffi.string(sqlite.sqlite3_value_text(x)).decode(),
    lambda x: ffi.buffer(sqlite.sqlite3_value_blob(x), sqlite.sqlite3_value_bytes(x))[:].decode(),
    lambda x: None
])

_bind_param = {
    bytes: lambda st, idx, param: sqlite.sqlite3_bind_text(st, idx, param, -1, SQLITE_TRANSIENT),
    str: lambda st, idx, param: sqlite.sqlite3_bind_text(st, idx, param.encode(), -1, SQLITE_TRANSIENT),
    ...
    memoryview: lambda st, idx, param: sqlite.sqlite3_bind_blob(st, idx, str(param), len(param), SQLITE_TRANSIENT)
}
...
_sqlite_to_python_statement = tuple([
    ...
    lambda st, ci: ffi.string(sqlite.sqlite3_column_text(st, ci)).decode(),
    lambda st, ci: ffi.buffer(sqlite.sqlite3_column_blob(
        st, ci), sqlite.sqlite3_column_bytes(st, ci))[:].decode(),
    lambda st, ci: None
])
...
_python_to_sqlitedict = {
    ...
    str: lambda c, val: sqlite.sqlite3_result_text(c, val.encode(), -1, SQLITE_TRANSIENT),
